...
class PsModel:
    ...
    """
    def __init__(self, source: Dict[str, float], event_model: EventModel) -> None:
        """Function info...
    ...
    def background_spatial_pdf(self, events: np.ndarray) -> np.array:
        ...
        """
        return (1/(2*np.pi))*self.event_model.background_dec_spline(np.sin(events['dec']))
    ...
    def reduced_sim(self,
        ...
        if sampling_width is not None:
            sindec_dist = np.abs(self.source['dec']-self.event_model.sim['trueDec'])
            close = sindec_dist < sampling_width
            
            reduced_sim = rf.append_fields(
                self.event_model.sim[close].copy(),
                'weight',
                np.zeros(close.sum()),
                dtypes=np.float32)
            ...
            reduced_sim = rf.append_fields(
                self.event_model.sim.copy(),
                'weight',
                np.zeros(len(self.event_model.sim)),
                dtypes=np.float32)
            omega = 4*np.pi
    ...
    def get_log_sob_gamma_splines(self, events: np.ndarray) -> List[scipy.interpolate.UnivariateSpline]:
        return self.event_model.get_log_sob_gamma_splines(events)
    
    @property
    def data(self) -> np.ndarray:
        return self.event_model.data
    
    @property
    def sim(self) -> np.ndarray:
        return self.event_model.sim
    
    @property
    def grl(self) -> np.ndarray:
        return self.event_model.grl