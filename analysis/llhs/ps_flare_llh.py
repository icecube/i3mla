class ps_flare_llh(object):
    ...
    def __init__(self, data, sim, grl, gammas, bins):
        ...
        self.bins     = bins
        self.gammas   = gammas
        self.sob_maps = np.zeros((len(bins[0]), len(bins[1]), len(gammas)))
        self.bg_p_dec = self._create_bg_p_dec()

        for i,gamma in enumerate(gammas):
            self.sob_maps[:,:,i] = self._create_interpolated_ratio(gamma, bins)
    ...
    def _get_energy_splines(self, events):
        ...
        sob_ratios = self.evaluate_interpolated_ratios(events)
    ...
    def produce_trial(self, N=0, gamma=-2, source = {'ra':np.pi/2, 'dec':np.pi/6},
                      ...
                      random_seed = None, signal_weights = None, sim = self.sim,
        ...
        if N > 0:
            if sim == self.sim:
                reduced_sim = self.select_and_weight(N=N,
                                                     gamma=gamma,
                                                     source = source,
    ...
    def evaluate_ts(self, events, source, bg_time_profile, sig_time_profile, ns = 0,
        ...
        S = self.signal_pdf(events, source['ra'], source['dec'])
        B = self.background_pdf(events, source['ra'], source['dec'])

        splines = self.get_energy_splines(events)
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            ...
            def get_inner_ts(params):
                ...
                e_lh_ratio = self.get_energy_sob(events, gamma, splines)
            ...
            if minimize:
                ...
                x0 = [ns,]
                bounds = [[0, N],]
                e_lh_ratio = self.get_energy_sob(events, bf_params.x[0], splines)
